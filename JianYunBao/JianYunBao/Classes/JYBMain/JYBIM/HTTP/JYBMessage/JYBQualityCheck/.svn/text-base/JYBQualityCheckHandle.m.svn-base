...
#import "JYBQualityCheckHandle.h"
#import "JYBIMSendQCCommentMessageApi.h"
#import "JYBIMSendQCNodeMessageApi.h"

@implementation JYBQualityCheckHandle
{
    JYBIMSendQCCommentMessageApi *commentApi;
    JYBIMSendQCNodeMessageApi *nodeApi;
}
...
- (instancetype)initSendBillChatMessageWithEnterpriseCode:(NSString *)enterpriseCode userid:(NSString *)userId name:(NSString *)name orderId:(NSString *)orderId nodeId:(NSString *)nodeId createDt:(NSString *)createDt coordinate:(CLLocationCoordinate2D)coordinate
{
    ...
    if (self) {
        commentApi = [[JYBIMSendQCCommentMessageApi alloc] initQualityCheckCommentApiWithEnterpriseCode:enterpriseCode userName:name userid:userId orderId:orderId];
        nodeApi = [[JYBIMSendQCNodeMessageApi alloc] initQualityCheckNoteApiWithEnterpriseCode:enterpriseCode userName:name userid:userId orderId:orderId nodeId:nodeId createDt:createDt coordinate:coordinate];
    }
    return self;
}
    ...
    if (self.billType == JYBBillTypeComment) {
        ...
        [commentApi startWithCompletionBlockWithSuccess:^(__kindof YTKBaseRequest *request) {
            ...
            if ([request.responseJSONObject[@"result"] integerValue] == 1 && ![request.responseJSONObject[@"deleted"] boolValue]) {
                ...
                entity.commandID = BD_TCP_PROTOCOL_CODE_QUALITYCHECK_COMMENT_TEXT_MESSAGE;
        ...
        [nodeApi startWithCompletionBlockWithSuccess:^(__kindof YTKBaseRequest *request) {
            ...
            if ([request.responseJSONObject[@"result"] integerValue] == 1 && ![request.responseJSONObject[@"deleted"] boolValue]) {
                JYBIChatMessage *entity = [[JYBIChatMessage alloc] init];
                entity.commandID = BD_TCP_PROTOCOL_CODE_QUALITYCHECK_RESULT_TEXT_MESSAGE;
                entity.messageID = request.responseJSONObject[@"id"];
                entity.fromUserID = self.sendUserId;
                entity.userName = self.sendUserName;
                entity.conversationChatter = self.groupId;
    ...
    if (self.billType == JYBBillTypeComment) {
        ...
        [commentApi startWithCompletionBlockWithSuccess:^(__kindof YTKBaseRequest *request) {
            ...
            if ([request.responseJSONObject[@"result"] intValue] == 1 && ![request.responseJSONObject[@"deleted"] boolValue]) {
                JYBIChatMessage *entity = [[JYBIChatMessage alloc] init];
                entity.commandID = BD_TCP_PROTOCOL_CODE_QUALITYCHECK_COMMENT_AUDIO_MESSAGE;
                entity.messageID = request.responseJSONObject[@"id"];
                entity.fromUserID = self.sendUserId;
                entity.userName = self.sendUserName;
                entity.conversationChatter = self.groupId;
        ...
        [nodeApi startWithCompletionBlockWithSuccess:^(__kindof YTKBaseRequest *request) {
            ...
            if ([request.responseJSONObject[@"result"] intValue] == 1 && ![request.responseJSONObject[@"deleted"] boolValue]) {
                JYBIChatMessage *entity = [[JYBIChatMessage alloc] init];
                entity.commandID = BD_TCP_PROTOCOL_CODE_QUALITYCHECK_RESULT_AUDIO_MESSAGE;
                entity.messageID = request.responseJSONObject[@"id"];
                entity.fromUserID = self.sendUserId;
                entity.userName = self.sendUserName;
                entity.conversationChatter = self.groupId;
...
- (void)sendImage:(UIImage *)image success:(void (^)(id))success failure:(void (^)(NSString *))failure
{
    ...
    if (self.billType == JYBBillTypeComment) {
        ...
        [commentApi startWithCompletionBlockWithSuccess:^(__kindof YTKBaseRequest *request) {
            ...
            if ([request.responseJSONObject[@"result"] intValue] == 1 && ![request.responseJSONObject[@"deleted"] boolValue]) {
                JYBIChatMessage *entity = [[JYBIChatMessage alloc] init];
                entity.commandID = BD_TCP_PROTOCOL_CODE_QUALITYCHECK_COMMENT_IMAGE_MESSAGE;
                entity.messageID = request.responseJSONObject[@"id"];
                entity.fromUserID = self.sendUserId;
                entity.userName = self.sendUserName;
                entity.conversationChatter = self.groupId;
        ...
        [nodeApi startWithCompletionBlockWithSuccess:^(__kindof YTKBaseRequest *request) {
            ...
            if ([request.responseJSONObject[@"result"] intValue] == 1 && ![request.responseJSONObject[@"deleted"] boolValue]) {
                JYBIChatMessage *entity = [[JYBIChatMessage alloc] init];
                entity.commandID = BD_TCP_PROTOCOL_CODE_QUALITYCHECK_RESULT_IMAGE_MESSAGE;
                entity.messageID = request.responseJSONObject[@"id"];
                entity.fromUserID = self.sendUserId;
                entity.userName = self.sendUserName;
                entity.conversationChatter = self.groupId;
...
- (void)sendVideo:(NSString *)filePath duration:(NSInteger)duration success:(void (^)(id))success failure:(void (^)(NSString *))failure
{
    ...
    if (self.billType == JYBBillTypeComment) {
        ...
        [commentApi startWithCompletionBlockWithSuccess:^(__kindof YTKBaseRequest *request) {
            ...
            if ([request.responseJSONObject[@"result"] intValue] == 1 && ![request.responseJSONObject[@"deleted"] boolValue]) {
                JYBIChatMessage *entity = [[JYBIChatMessage alloc] init];
                entity.commandID = BD_TCP_PROTOCOL_CODE_QUALITYCHECK_COMMENT_VIDEO_MESSAGE;
                entity.messageID = request.responseJSONObject[@"id"];
                entity.fromUserID = self.sendUserId;
                entity.userName = self.sendUserName;
                entity.conversationChatter = self.groupId;
        ...
        [nodeApi startWithCompletionBlockWithSuccess:^(__kindof YTKBaseRequest *request) {
            ...
            if ([request.responseJSONObject[@"result"] intValue] == 1 && ![request.responseJSONObject[@"deleted"] boolValue]) {
                JYBIChatMessage *entity = [[JYBIChatMessage alloc] init];
                entity.commandID = BD_TCP_PROTOCOL_CODE_QUALITYCHECK_RESULT_VIDEO_MESSAGE;
                entity.messageID = request.responseJSONObject[@"id"];
                entity.fromUserID = self.sendUserId;
                entity.userName = self.sendUserName;
                entity.conversationChatter = self.groupId;
