...
- (void)asyncPlayingWithPath:(NSString *)aFilePath
    ...
    if ([aFilePath hasSuffix:@"amr"]) {//播放本地录制的音频
        ...
        if (completion) {
            if (recordPath) {
                ...
                NSString *amrFilePath = [[recordPath stringByDeletingPathExtension]
                                         stringByAppendingPathExtension:@"amr"];//amr
                BOOL convertResult = [self convertWAV:recordPath toAMR:amrFilePath];
//                BOOL convertResult = [self convertWAV:recordPath toMP3:amrFilePath];
                if (convertResult) {
                    // 删除录的wav
                    NSFileManager *fm = [NSFileManager defaultManager];
