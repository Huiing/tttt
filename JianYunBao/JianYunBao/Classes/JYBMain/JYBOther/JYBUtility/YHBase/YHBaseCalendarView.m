...
-(void)reloadView{
    ...
    _scrollView = [[UIScrollView alloc]initWithFrame:CGRectMake(0, 20, self.frame.size.width, self.frame.size.height)];
    ...
    _bodyViewL.backgroundColor = [UIColor whiteColor];
    ...
    _bodyViewM.backgroundColor = [UIColor whiteColor];
    ...
    _bodyViewR.backgroundColor = [UIColor whiteColor];
    [_scrollView addSubview:_bodyViewR];
    
    
    _headView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, self.frame.size.width, 20)];
    _headView.backgroundColor = [UIColor redColor];
    NSArray * weekArray = @[@"周日",@"周一",@"周二",@"周三",@"周四",@"周五",@"周六"];
    for (int i=0; i<7; i++) {
        UILabel * label = [[UILabel alloc]initWithFrame:CGRectMake(self.frame.size.width/7*i, 0, self.frame.size.width/7, 20)];
//        if (i!=0&&i!=6) {
//            label.backgroundColor = [UIColor redColor];
//        }else{
//            label.backgroundColor = [UIColor purpleColor];
//        }
        label.backgroundColor = [UIColor colorWithRed:130/255.0 green:145/255.0 blue:167/255.0 alpha:1];
        label.text=weekArray[i];
        label.textAlignment = NSTextAlignmentCenter;
        label.layer.borderWidth=1;
        label.layer.borderColor = [[UIColor colorWithRed:130/255.0 green:145/255.0 blue:167/255.0 alpha:1]CGColor];
    ...
    for (int i=0; i<weekRow; i++) {
        ...
        for (int j=0; j<7; j++) {
            ...
            if (weekday!=7&&(i*7+j)<weekday) {
                ...
                btn.layer.borderColor = [UIColor colorWithRed:223/255.0 green:223/255.0 blue:223/255.0 alpha:1].CGColor;
            ...
            if ([currentDate getYear]==[_today getYear]&&[currentDate getMonth]==[_today getMonth]&&[btn.titleLabel.text intValue]==[_today getDay]&&!CGColorEqualToColor([btn.titleLabel.textColor CGColor], [[UIColor grayColor] CGColor])) {
                [btn setTitleColor:[UIColor colorWithRed:53/255.0 green:149/255.0 blue:206/255.0 alpha:1] forState:UIControlStateNormal];
                [btn setBackgroundImage:[UIImage imageNamed:@"选中1"] forState:UIControlStateNormal];
            }
            ...
            if (_markArray!=nil) {
                for (int i=0; i<_markArray.count; i++) {
                    ...
                    if ([currentDate getYear]==[model.year intValue]&&[currentDate getMonth]==[model.month intValue]&&[btn.titleLabel.text intValue]==[model.day intValue]&&!CGColorEqualToColor([btn.titleLabel.textColor CGColor], [[UIColor grayColor] CGColor])) {
                        if ([currentDate getYear]==[_today getYear]&&[currentDate getMonth]==[_today getMonth]&&[btn.titleLabel.text intValue]==[_today getDay]&&!CGColorEqualToColor([btn.titleLabel.textColor CGColor], [[UIColor grayColor] CGColor])) {
                            [btn setTitleColor:[UIColor colorWithRed:53/255.0 green:149/255.0 blue:206/255.0 alpha:1] forState:UIControlStateNormal];
                            [btn setBackgroundImage:[UIImage imageNamed:@"选中4"] forState:UIControlStateNormal];
                        }else if ([_selectModel.year intValue]==[currentDate getYear]&&[_selectModel.month intValue]==[currentDate getMonth]&&[_selectModel.day intValue]==[btn.titleLabel.text intValue]&&!CGColorEqualToColor([btn.titleLabel.textColor CGColor], [[UIColor grayColor] CGColor])) {
                            //是否进行选中标记
                            [btn setBackgroundImage:[UIImage imageNamed:@"选中3"] forState:UIControlStateNormal];
                        }else{
                            [btn setBackgroundImage:[UIImage imageNamed:@"选中2"] forState:UIControlStateNormal];
                            btn.layer.borderWidth = 0.5;
                            btn.layer.borderColor = [UIColor colorWithRed:223/255.0 green:223/255.0 blue:223/255.0 alpha:1].CGColor;
                        }
                        UILabel *numLabel = [[UILabel alloc] initWithFrame:CGRectMake(btn.width - 15, 3, 15, 8)];
