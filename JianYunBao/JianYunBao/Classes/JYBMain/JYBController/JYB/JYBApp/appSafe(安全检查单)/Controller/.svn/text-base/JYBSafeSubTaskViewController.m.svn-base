//
//  JYBSafeSubTaskViewController.m
//  JianYunBao
//
...
@interface JYBSafeSubTaskViewController ()<UITableViewDelegate,UITableViewDataSource>
{
...
@implementation JYBSafeSubTaskViewController
...
- (void)viewDidLoad {
    ...
    taskHeadView.numberTextField.text = _safeSubtaskItem.number;
    taskHeadView.taskNameTextField.text = _safeSubtaskItem.name;
    taskHeadView.describeTextField.text = _safeSubtaskItem.note;
...
{
    SYWCommonRequest *api = [[SYWCommonRequest alloc] init];
    api.ReqURL = JYBBCHttpUrl(@"/phone/SafetyInspectNodeUsers!addUsers.action");
    if(isSubstract)
    {
        api.ReqURL = JYBBCHttpUrl(@"/phone/SafetyInspectNodeUsers!removeUsers.action");
    }
    api.ReqDictionary =
    @{
      @"orderId":_safeSubtaskItem.safetyId,
      @"nodeId":_safeSubtaskItem.safeSubTaskid,
