//
//  JYBQualityOrderMainViewController.m
//  JianYunBao
//
...
@interface JYBQualityOrderMainViewController ()<SelectIndexDelegate,UITableViewDataSource,UITableViewDelegate,ToastViewDelegate,SelectStatusViewDelegate,SelectOrderStatusViewDelegate>
...
@implementation JYBQualityOrderMainViewController
...
- (void)viewDidLoad {
    ...
    JYBWorkCommunicationViewController *vc = [[JYBWorkCommunicationViewController alloc] initWithChatter:self.qualityOrderId type:JYBConversationTypeSingle];
...
- (void)checkUpdateOrder
{
    NSString *userId = [RuntimeStatus sharedInstance].userItem.userId;
    SYWCommonRequest *api = [[SYWCommonRequest alloc] init];
    api.ReqURL = JYBBCHttpUrl(@"/phone/QualityInspectUpdate!update.action");
    api.ReqDictionary =
    @{
      @"userId":userId,
      @"orderId":self.qualityOrderId,
      @"version":@"0",
      @"attachmentVersion":@"0",
      @"nodeVersion":@"0"
      };
    [api startWithCompletionBlockWithSuccess:^(YTKBaseRequest *request) {
        ...
        if (GoodResponse) {
            ...
            for(NSDictionary *node in nodes)
            {
                ...
                JYBQualitySubtaskItem *item = [JYBQualitySubtaskItem mj_objectWithKeyValues:nodeDic];
...
{
    SYWCommonRequest *api = [[SYWCommonRequest alloc] init];
    api.ReqURL = JYBBCHttpUrl(@"/phone/QualityInspectNodeCreate!create.action");
    api.ReqDictionary =
    @{
      @"orderId":self.qualityOrderId,
...
- (void)getNodeData
{
    NSString *userId = [RuntimeStatus sharedInstance].userItem.userId;
    SYWCommonRequest *api = [[SYWCommonRequest alloc] init];
    api.ReqURL = JYBBCHttpUrl(@"/phone/QualityInspectNode!get.action");
    api.ReqDictionary =
    @{
      @"po.orderId":self.qualityOrderId,
      @"po.id":@"10"
...
- (void)confirmStatusAction
{
    NSString *userId = [RuntimeStatus sharedInstance].userItem.userId;
    SYWCommonRequest *api = [[SYWCommonRequest alloc] init];
    api.ReqURL = JYBBCHttpUrl(@"/phone/QualityInspectNodeState!state.action");
    api.ReqDictionary =
    @{
      @"userId":userId,
      @"orderId":self.qualityOrderId,
...
- (void)confirmOrderStatusAction
{
    NSString *userId = [RuntimeStatus sharedInstance].userItem.userId;
    SYWCommonRequest *api = [[SYWCommonRequest alloc] init];
    api.ReqURL = JYBBCHttpUrl(@"/phone/QualityInspect!state.action");
    api.ReqDictionary =
    @{
      @"userId":userId,
      @"po.id":self.qualityOrderId,
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(tableView.tag == 0)
    {
        JYBQualityTaskCell *cell = [JYBQualityTaskCell cellWithTableView:tableView indexPath:indexPath];
        cell.backgroundColor = RGB(240, 240, 240);
        JYBQualitySubtaskItem *item = nodeArray[indexPath.row];
        
        //    item.executors = executorsArr[indexPath.row];
        [cell setQualitySubtaskItem:item];
        if(row == 0)
        {
        ...
        cell.statusImageBlock = ^{
            ...
            statImg = cell.workStateImage;
            nodeId = item.qualitySubTaskid;
            if(!_setstatusView)
            {
                _setstatusView = [[JYBSetStatusView alloc] initWithFrame:CGRectMake(0, 0, Mwidth, Mheight)];
                _setstatusView.state = item.workState;
        ...
        cell.lookImageBlock = ^{
            JYBQualitySubTaskViewController *vc = [[JYBQualitySubTaskViewController alloc] init];
            vc.navigationTitle = @"工单子任务";
            vc.qualitySubtaskItem = item;
    ...
    else if (tableView.tag ==2)
    {
        ...
        else
        {
            ...
            else if(indexPath.row ==3)
            {
                ...
                icon.image = [QRCodeGenerator qrImageForString:[NSString stringWithFormat:@" jyb:2010:%@",self.qualityOrderId] imageSize:icon.bounds.size.width];
...
{
    SYWCommonRequest *api = [[SYWCommonRequest alloc] init];
    api.ReqURL = JYBBCHttpUrl(@"/phone/QualityInspectUsers!addUsers.action");
    if(!isAdd)
    {
        api.ReqURL = JYBBCHttpUrl(@"/phone/QualityInspectUsers!removeUsers.action");
    }
    api.ReqDictionary =
    @{
      @"orderId":self.qualityOrderId,
