...
#import "JYBTaskSourceItem.h"
@interface JYBSelectTaskViewController ()<UISearchBarDelegate,UITableViewDataSource,UITableViewDelegate>
{
...
@implementation JYBSelectTaskViewController
...
- (void)loadData
{
  ...
  api.ReqURL = JYBBCHttpUrl(@"/phone/SysAttr!someWorkType.action");
  api.ReqDictionary =
  @{
  ...
  [api startWithCompletionBlockWithSuccess:^(YTKBaseRequest *request) {
    ...
    if (GoodResponse) {
      _taskItems = [JYBTaskSourceItem mj_objectArrayWithKeyValuesArray:APIJsonObject[@"list"]];
      for(JYBTaskSourceItem *item in _taskItems)
      {
...
{
  UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"提示" message:@"确定选择该任务来源" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
  [alertView show];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
  ...
  else
  {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"SelctedTaskNotifiaction" object:@[name,typeCode] userInfo:nil];
